...
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedClassFound
    class CCWE_Settings
    {
        ...
        {
            wp_enqueue_script('ccew-admin-script', CCEW_URL . 'assets/js/admin-script.js', array('jquery'), CCEW_VERSION, true);
        ...
        public function ccew_reg_settings()
        {
            register_setting(
                ...
                'ccew_sanitize_api_settings' // Sanitize
            );
        ...
        public function ccew_purge_cache()
        {
            ...
            echo ' <button class="button button-secondary ccew-delete-transient" data-ccpw-nonce="' . esc_attr($ccew_nonce) . '" data-ajax-url="' . esc_url($ajax_url) . '" id="ccew-delete-transient">' . esc_html__('Purge Cache', 'cryptocurrency-widgets-for-elementor') . '</button>';
        }
...
<?php
            ...
            $get_setings = (isset($_GET['_wpnonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'])), 'ccew_settings_nonce'))
                ? sanitize_key(wp_unslash($_GET['page'] ?? ''))
                : '';

            $update_setings = sanitize_text_field(wp_unslash($_GET['settings-updated'] ?? ''));
        ...
        public function ccew_delete_transient()
        {
            ...
            if (
                ...
                wp_die(esc_html__('You don\'t have permission to delete the cache.', 'cryptocurrency-widgets-for-elementor'));
            }
